...
GSM::GSM(bool debug) : _state(ERROR),
                       _readyState(0),
                       _pin(NULL),
                       _timeout(0)
{
...
bool GSM::shutdown()
{
  if (_state == GSM_READY)
  {
    MODEM.send("AT+CPWROFF"); // por comando
    MODEM.waitForResponse();  // En Sara 3G el "The +UCPWROFF URC is not supported."
  }
  MODEM.end();
  _state = GSM_OFF;
  return true;
