...
GSMClient::GSMClient(bool synch) : GSMClient(-1, synch)
{
}

GSMClient::GSMClient(int socket, bool synch) : _synch(synch),
                                               _socket(socket),
                                               _connected(false),
...
size_t GSMClient::writeBinary(const uint8_t *buf, size_t size)
{
  if (_writeSync)
  {
    while (ready() == 0)
  ...
  while (size)
  {
    ...
    MODEM.write(&buf[index], chunkSize);
    if (_writeSync)
    {
      if (MODEM.waitForResponse(10000) != 1)
      {
