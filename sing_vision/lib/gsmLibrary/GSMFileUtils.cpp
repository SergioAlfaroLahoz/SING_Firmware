...
uint32_t GSMFileUtils::downloadFileBinary(const String filename, uint8_t buf[], uint32_t size, const bool append)
{
    if (!append)
        deleteFile(filename);
...
uint32_t GSMFileUtils::readFileBinary(const String filename, uint8_t *content)
{
    if (listFile(filename) == 0)
    {
        return 0;
    ...
    MODEM.sendf("AT+URDFILE=\"%s\"", filename.c_str());
    int byteRead = MODEM.waitForEndReadFile(10000, content);
    return byteRead;
}
