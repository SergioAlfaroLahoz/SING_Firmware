...
GSMHttpUtils::GSMHttpUtils() : _gsmRoots(GSM_ROOT_CERTS),
...
void GSMHttpUtils::configHeaders(int headerIndex, const char *header)
{
  // profile #0
  MODEM.sendf("AT+UHTTP=0,9,\"%i:%s\"", headerIndex, header);
  MODEM.waitForResponse(100);
}
...
/**
 * Send a HTTP POST request with a file as data and store the response in a file
 **/
void GSMHttpUtils::postFile(const char *path, const char *resultfilename, const char *dataFilename)
{
  _httpresp = false;
  MODEM.sendf("AT+UHTTPC=0,4,\"%s\",\"%s\",\"%s\",3", path, resultfilename, dataFilename);
  MODEM.waitForResponse(200);
}

/**
 * Send a HTTP POST request with data (application/x-www-form-urlencoded) and store the response in a file
  **/
void GSMHttpUtils::postData(const char *path, const char *resultfilename, const char *data)
{
  // make a POST request and store it in _file
  _httpresp = false;
  MODEM.sendf("AT+UHTTPC=0,5,\"%s\",\"%s\",\"%s\",0", path, resultfilename, data);
  MODEM.waitForResponse(200);
}
