...
void loop()
{
  ...
  if (currentMillis - previousMillis >= timerInterval)
  {
    ...
    else
    {
      ...
      char postParameter[128];                                            // Solo se pueden añadir 128 bytes como parametros
      sprintf(postParameter, "USER_ID=%s&EAN=%s&METAL_READ=%i&PET_READ=%i", "INAR", "ZaC", 1, 0);
      httpClient.postData(path, filename, postParameter);
