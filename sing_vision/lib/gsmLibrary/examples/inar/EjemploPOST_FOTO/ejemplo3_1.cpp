...
const int timerInterval = 120000; // Tiempo entre peticiones en ms
...
void setup()
{
  ...
  Serial.println("============================");
  Serial.println("Ejemplo 3.2 Realiza una peticion HTTP POST periodicamente");
...
void loop()
{
  ...
  if (currentMillis - previousMillis >= timerInterval)
  {
    ...
    else
    {
      ...
      fileUtils.downloadFileBinary(filenamePicture, fb->buf, fb->len, false); // se carga el fichero a enviar en la memoria del transceiver
      httpClient.postFile(path, filename, filenamePicture);                   //lanzamos la peticion de post
